...
type SearchRequestPayload struct {
	...
	Criteria     HotelSearchCriteria `json:"criteria"`
}
...
type OfferResponse struct {
	...
	Details        HotelOfferDetails `json:"details"`
}
...
func (s *Service) SearchOffers(ctx context.Context, req SearchRequestPayload, now time.Time) (*SearchHotelResponse, []platform.ErrorDetail, error) {
	if req.ProviderType != "HOTEL" {
		return nil, []platform.ErrorDetail{{Field: "provider_type", Reason: "must be HOTEL"}}, ErrInvalidRequest
	}

	crit := req.Criteria
	var details []platform.ErrorDetail

	if !platform.ValidateDateFormat(crit.CheckInDate) {
		details = append(details, platform.ErrorDetail{Field: "criteria.check_in_date", Reason: "must be in YYYY-MM-DD date format"})
	}
	if !platform.ValidateDateFormat(crit.CheckOutDate) {
		details = append(details, platform.ErrorDetail{Field: "criteria.check_out_date", Reason: "must be in YYYY-MM-DD date format"})
	}
	if !platform.ValidateIanaTimezone(crit.LocalTimezone) {
		details = append(details, platform.ErrorDetail{Field: "criteria.local_timezone", Reason: "must be a valid IANA time zone"})
	}
	if crit.RoomCount < 1 || crit.RoomCount > 10 {
		details = append(details, platform.ErrorDetail{Field: "criteria.room_count", Reason: "must be between 1 and 10"})
	}
	if crit.GuestCount < 1 || crit.GuestCount > 20 {
		details = append(details, platform.ErrorDetail{Field: "criteria.guest_count", Reason: "must be between 1 and 20"})
	}
	...
	checkIn, _ := time.Parse("2006-01-02", crit.CheckInDate)
	checkOut, _ := time.Parse("2006-01-02", crit.CheckOutDate)
	if !checkIn.Before(checkOut) {
		return nil, []platform.ErrorDetail{{Field: "criteria.check_out_date", Reason: "check_out_date must be after check_in_date"}}, ErrInvalidRequest
	}
	...
	for _, item := range offersWithUnits {
		...
		accessibility := []string(roomType.Accessibility)
		...
		offers = append(offers, OfferResponse{
			...
			Details: HotelOfferDetails{
				ProviderType:     "HOTEL",
				RoomInventoryID:  offer.ExternalInventoryID,
				PropertyName:     roomType.PropertyName,
				RoomType:         roomType.Name,
				CheckInDate:      offer.CheckInDate.Format("2006-01-02"),
				CheckOutDate:     offer.CheckOutDate.Format("2006-01-02"),
				MaxGuestsPerRoom: roomType.MaxGuestsPerRoom,
...
func (s *Service) CreateHold(
	...
	if req.ProviderID != s.providerID || req.ProviderType != "HOTEL" {
	...
	if req.BookingRequirements != nil && string(*req.BookingRequirements) != "null" && string(*req.BookingRequirements) != "" {
		details = append(details, platform.ErrorDetail{
			...
			Reason: "is rejected for HOTEL provider type",
		})
	}

	if req.Units < 1 || req.Units > 10 {
		details = append(details, platform.ErrorDetail{
			Field:  "units",
